  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h3 className="font-display font-semibold text-[var(--color-text)]">Manajemen Akun</h3>
          <p className="text-xs text-[var(--color-text-muted)]">Total {users.length} akun terdaftar</p>
        </div>
          ...
          <tbody>
            {users.map((u) => (
              <tr key={u.id} className="table-row">
                <td className="font-medium text-[var(--color-text)]">{u.nama}</td>
                <td>{u.email}</td>
                <td>
                ...
                <td className="text-right">
                  <button onClick={() => handleToggleStatus(u)} className={`text-xs hover:underline ${u.is_active ? 'text-red-600' : 'text-green-600'}`}>
